...
class PetAnimal(models.Model):
    ...
    pet_code = fields.Char(
        string="Pet Chip/Code",
        copy=False,
        index=True,
    ...
    veterinarian_id = fields.Many2one(
        string="Veterinarian", comodel_name="hr.employee", tracking=True
    )
    ...
    vet_appointment_ids = fields.One2many(
        string="Veterinarian Appointments",
        comodel_name="veterinary.appointment",
        inverse_name="animal_id",
    )
    vet_apmt_count = fields.Integer(
        string="Amount Vet Appointment",
        compute="_compute_vet_appointment_count",
        store=True,
    )

    _sql_constraints = [
        (
            "pet_animal_unique_code",
            "UNIQUE (pet_code)",
