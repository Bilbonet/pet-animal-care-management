...
class CreateInvoice(models.TransientModel):
    _name = "day_care_create.invoice"

    day_care_id = fields.Many2one('pet.day_care',string='Pet Day Care Id')
    ...
    day_care_ids = fields.Many2many('day_care.pets',string='Pets')
    ...
    def default_get(self, fields):
        ...
        day_care = self.env['pet.day_care'].search([('id','=',active_ids)])
        amount = 0
        pet_list = []
        if day_care:
             for record in day_care:
                 if record:
                     for pet in record.day_care_pet_ids:
                         if pet:
                             pet_list.append(pet.id)
                     rec.update({
                                 'day_care_id':record.id,
                                 'partner_id':record.partner_id.id,
                                 'day_care_ids':[(6, 0, pet_list)],  
                                 'total_amount':record.total_amount,
                                })   
        return rec

    @api.depends('day_care_ids') 
    def count_total_pets(self):
        count = 0
        for pets in self.day_care_ids:
            if pets:
                count = count + 1
    ...
    def create_invoice(self):
        ...
        active_ids = self._context.get('active_ids')
        day_care = self.env['pet.day_care'].browse(active_ids)
        product = self.env.ref('abs_pet_animal_care_management.charges_day_care')
        ...
        for record in self:
            if record:
                invoice_dict = {
                                ...
                                'name':record.day_care_id.number,
                               }
                if invoice_dict:
